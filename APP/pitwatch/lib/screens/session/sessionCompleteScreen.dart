...
class _SessionCompleteScreenState extends ConsumerState<SessionCompleteScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Capture the initial session hazards count so the stat card
      // remains stable even after we migrate successful uploads
      // into the global provider.
      final sessionDetections = ref.read(sessionPotholesProvider);
      setState(() {
        _initialHazards = sessionDetections.isNotEmpty
            ? sessionDetections.length
            : widget.hazards;
      });
      _startUpload();
    });
  ...
  Future<void> _startUpload() async {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      if (ok) {
        try {
          // Persist the successfully uploaded detection so provider totals
          // update and the Home screen reflects the new count.
          // Prefer using the server's response object when available so
          // the provider receives the authoritative record (with id,
          // created_at, etc.). Fall back to the parsed local object.
          if (res['data'] is Map<String, dynamic>) {
            try {
              final Map<String, dynamic> serverMap = Map<String, dynamic>.from(
                res['data'],
              );
              // Merge local severity (if present) into server response so
              // provider retains the classification made on-device.
              if (i < parsed.length) {
                try {
                  final localJson = parsed[i].toJson();
                  if (localJson.containsKey('severity')) {
                    serverMap['severity'] = localJson['severity'];
                  }
                } catch (_) {}
              }
              try {
                final pd = PotholeDetection.fromJson(serverMap);
        ...
        // re-add original map for retry later (use parsed[i] to keep index
        // alignment with `results` which was produced from `parsed`).
        try {
          if (i < parsed.length) failedMaps.add(parsed[i].toJson());
        } catch (_) {}
    ...
    // If everything uploaded successfully, ensure session storage
    // and the displayed session count are cleared to zero.
    if (failedMaps.isEmpty && _total > 0 && _uploaded == _total) {
      try {
        ref.read(sessionPotholesProvider.notifier).clear();
      } catch (_) {}
      try {
        // also clear any notifier-held session cache (backing notifier)
        ref.read(potholeProvider.notifier).clearSession();
      } catch (_) {}
      if (mounted) {
        setState(() {
          _initialHazards = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final listDetections = sessionDetections.isNotEmpty
        ? sessionDetections
        : widget.detections;
    // Use the captured initial hazards count if available so the displayed
    // number doesn't change when we migrate successful uploads out of the
    // session provider.
    final hazardsCount =
        _initialHazards ??
        (sessionDetections.isNotEmpty
            ? sessionDetections.length
            : widget.hazards);
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    /// DETECTIONS LIST (grouped by severity)
                    if (listDetections.isNotEmpty) ...[
                      Align(
                      ...
                      // Parse detections into model objects and group by severity
                      Builder(
                        builder: (ctx) {
                          final parsed = <PotholeDetection>[];
                          for (final d in listDetections) {
                            try {
                              parsed.add(
                          ...
                          final high = parsed
                              .where((p) => p.severity == Severity.high)
                              .toList();
                          final medium = parsed
                              .where((p) => p.severity == Severity.medium)
                              .toList();
                          final low = parsed
                              .where((p) => p.severity == Severity.low)
                              .toList();
                          final unknown = parsed
                              .where((p) => p.severity == Severity.unknown)
                              .toList();
                          ...
                          return Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              if (high.isNotEmpty) buildSection('High', high),
                              if (medium.isNotEmpty)
                                buildSection('Medium', medium),
                              if (low.isNotEmpty) buildSection('Low', low),
                              if (unknown.isNotEmpty)
                                buildSection('Uncategorized', unknown),
                            ],
                          );
                        },
                      ),
